...
const userModel = require('./dbModels/userModel.js');
const sessionModel = require('./dbModels/sessionModel.js');
...
connection.once(
  ...
  () => { 
    ...
    // userModel.find({ username: "weqweweqeq" }, (err, users) => { 
...
app.post('/register', (req, res) => {
  ...
  bcrypt.hash(uModelObj.password, saltRounds, function(err, hash) {
    ...
    let newUser = new userModel(uModelObj);
...
app.post('/login', (req, res) => {
  ...
  userModel.findOne({ username: loginCreds.username }, (err, user) => { 
    if (err) return console.error(err);
    // console.log(user);
    if(user){
  ...
  var createJWT = (user) => {
    ...
    sessionModel.findOne({ user: user.username }, (err, dbtoken) => {
      if (err) return console.error(err);
      ...
      if(dbtoken){
        ...
        let newToken = new sessionModel(tokenobj);
...
const verifyToken = async (jwtToken) => {
  ...
  let result = await sessionModel.findOne({ jwt: jwtToken });
  let res = {};
  if(result){
